...
**Status:** M1 in progress, monorepo scaffolded, framework package started.
...
### M1: web renderer (IN PROGRESS)
Astro integration, Zod config, docs and blog collections, theme system, components, all dogfooded by making the current flect site consume the package.
...
- [ ] `themes/`: port flect's 42-theme registry, rename `flect-*` ids to `almanac-*`
- [ ] `vite/`: virtual module for config (`virtual:almanac/config`) and component overrides
- [ ] `integration.ts`: `astro:config:setup` (validate config, inject routes, register vite plugin, noExternal), `astro:config:done`, `astro:build:done` (Pagefind)
- [ ] Port components, layouts, styles from flect
- [ ] Route injection for docs and blog
- [ ] `www/` builds against the package
- [ ] `starter/` tree
- [ ] `create-almanac` CLI
