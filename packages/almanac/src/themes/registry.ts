/**
 * Theme registry. The default theme's tokens live directly in global.css as
 * `:root` / `:root[data-mode="dark"]` rules, so it emits no overrides. Every
 * other theme derives its full token set from a four-color seed via
 * buildTokens(), which keeps adding a theme to a handful of colors instead of
 * a dozen hand-tuned properties.
 */
...
export interface Theme {
	...
	swatch: string;
	/** Background, surface, and text for the picker preview tile. */
	bg: string;
	surface: string;
	text: string;
	/** CSS custom-property overrides. Empty for the default theme. */
...
function parse(color: string): RGBA {
	...
	const body = match?.[1];
	if (body) {
		const p = body.split(",").map((s) => Number.parseFloat(s.trim()));
...
export function buildTokens(seed: Seed): Record<string, string> {
...
interface Signature extends Seed {
	id: string;
	family: string;
	name: string;
}
...
/** The default look, whose tokens are hand-tuned in global.css. */
...
export const THEMES: Theme[] = [
	...MODES.map(almanacTheme),
	...FAMILIES.flatMap((f) => MODES.map((mode) => familyTheme(f, mode))),
	...SIGNATURES.map(signatureTheme),
];

export const DEFAULT_LIGHT_ID = "almanac-light";
export const DEFAULT_DARK_ID = "almanac-dark";
 ...
 * Falls back to the default theme when a family has no sibling mode.
 */
...
/**
 * Narrows the registry to the themes a site opted into. The default family is
 * always kept so there is something to fall back to.
 */
...
/**
 * CSS for every non-default theme, embedded once at build time. Selecting a
 * theme sets data-mode and data-theme on <html> and these rules take over.
 *
 * The selector matches BOTH attributes so its specificity (html plus two
 * attributes) unambiguously beats the base `:root[data-mode="dark"]` rule in
 * global.css (:root plus one attribute). Matching data-theme alone ties that
 * rule in dark mode and loses the tie-break, since stylesheet order depends on
 * how Vite emits them.
 */
export function buildThemesCss(themes: Theme[] = THEMES): string {
	return themes
		.filter((t) => Object.keys(t.tokens).length > 0)
		.map((t) => {
