...
/**
 * Page ids in the order an explicit sidebar lists them.
 *
 * Used to give the book its reading order. Autogenerated directories are skipped
 * rather than guessed at, so anything they would have contributed falls to the
 * sorted tail instead of appearing in an order nobody chose.
 */
export function sidebarOrder(sidebar?: SidebarItem[]): string[] {
	const ids: string[] = [];
